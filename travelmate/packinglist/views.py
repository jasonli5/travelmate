...
def create_item(request):
    ...
    if (
        ...
        if not request.POST.get("name"):
            print("- 'name' is missing/empty")
        if not request.POST.get("description"):
            print("- 'description' is missing/empty")
        return redirect("packing_list")


def edit_item(request, item_id):
    item = get_object_or_404(
        Item, id=item_id, user=request.user
...
def ai_suggest_items(request):
    if request.method == "POST":
        ...
        existing_items = list(
            Item.objects.filter(user=request.user).values("name", "description")
        )
...
def packing_list(request):
    ...
    user_items = Item.objects.filter(user=request.user, is_ai_suggested=False).order_by(
        "id"
    )
    ai_items = Item.objects.filter(user=request.user, is_ai_suggested=True).order_by(
        "id"
    )
